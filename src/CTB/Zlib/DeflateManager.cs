...
namespace Ionic.Zlib
{
  internal sealed class DeflateManager
  {
    ...
    internal int build_bl_tree()
    {
      ...
      while (index >= 3 && (int) this.bl_tree[(int) Tree.bl_order[index] * 2 + 1] == 0)
        --index;
    ...
    internal bool _tr_tally(int dist, int lc)
    {
      ...
      if ((this.last_lit & 8191) == 0 && this.compressionLevel > CompressionLevel.Level2)
      {
        ...
        if (this.matches < this.last_lit / 2 && num1 >> 3 < num2 / 2)
          return true;
      }
      ...
      if (this.last_lit != 0)
      {
        do
        {
          ...
          else
          {
            ...
            int length1 = Tree.ExtraLengthBits[index2];
            if (length1 != 0)
              this.send_bits(c1 - Tree.LengthBase[index2], length1);
            int dist = num2 - 1;
            int c2 = Tree.DistanceCode(dist);
            this.send_code(c2, dtree);
            int length2 = Tree.ExtraDistanceBits[c2];
            if (length2 != 0)
              this.send_bits(dist - Tree.DistanceBase[c2], length2);
          }
        }
    ...
    private void _fillWindow()
    {
      do
      {
        ...
        else if (this.strstart >= this.w_size + this.w_size - DeflateManager.MIN_LOOKAHEAD)
        {
          ...
          int num1 = this.hash_size;
          int index1 = num1;
          do
          {
            int num2 = (int) this.head[--index1] & (int) ushort.MaxValue;
            this.head[index1] = num2 >= this.w_size ? (short) (num2 - this.w_size) : (short) 0;
          }
          while (--num1 != 0);
          ...
          do
          {
            int num2 = (int) this.prev[--index2] & (int) ushort.MaxValue;
            this.prev[index2] = num2 >= this.w_size ? (short) (num2 - this.w_size) : (short) 0;
          }
          while (--num3 != 0);
          size += this.w_size;
        }
...
label_12:;
    ...
    internal BlockState DeflateFast(FlushType flush)
    {
      ...
      while (true)
      {
        ...
        if ((long) cur_match != 0L && (this.strstart - cur_match & (int) ushort.MaxValue) <= this.w_size - DeflateManager.MIN_LOOKAHEAD && this.compressionStrategy != CompressionStrategy.HuffmanOnly)
...
label_28:
    ...
    {
      int num1 = this.config.MaxChainLength;
      int index1 = this.strstart;
      int num2 = this.prev_length;
      int num3 = this.strstart > this.w_size - DeflateManager.MIN_LOOKAHEAD ? this.strstart - (this.w_size - DeflateManager.MIN_LOOKAHEAD) : 0;
      int num4 = this.config.NiceLength;
      int num5 = this.w_mask;
      int num6 = this.strstart + DeflateManager.MAX_MATCH;
      byte num7 = this.window[index1 + num2 - 1];
      byte num8 = this.window[index1 + num2];
      ...
      do
      {
        ...
        if ((int) this.window[index2 + num2] == (int) num8 && (int) this.window[index2 + num2 - 1] == (int) num7 && (int) this.window[index2] == (int) this.window[index1] && (int) this.window[++index2] == (int) this.window[index1 + 1])
        {
          int num9 = index1 + 2;
          int num10 = index2 + 1;
          do
            ;
          while ((int) this.window[++num9] == (int) this.window[++num10] && (int) this.window[++num9] == (int) this.window[++num10] && ((int) this.window[++num9] == (int) this.window[++num10] && (int) this.window[++num9] == (int) this.window[++num10]) && ((int) this.window[++num9] == (int) this.window[++num10] && (int) this.window[++num9] == (int) this.window[++num10] && ((int) this.window[++num9] == (int) this.window[++num10] && (int) this.window[++num9] == (int) this.window[++num10])) && num9 < num6);
          int num11 = DeflateManager.MAX_MATCH - (num6 - num9);
          index1 = num6 - DeflateManager.MAX_MATCH;
          if (num11 > num2)
          {
            ...
            if (num11 < num4)
            {
              num7 = this.window[index1 + num2 - 1];
              num8 = this.window[index1 + num2];
            }
            else
              break;
          }
      ...
      while ((cur_match = (int) this.prev[cur_match & num5] & (int) ushort.MaxValue) > num3 && --num1 != 0);
    ...
    internal int Deflate(FlushType flush)
    {
      ...
      if (this.status == DeflateManager.INIT_STATE)
      {
        ...
        if (this.strstart != 0)
          num4 |= DeflateManager.PRESET_DICT;
        int num5 = num4 + (31 - num4 % 31);
        this.status = DeflateManager.BUSY_STATE;
        this.pending[this.pendingCount++] = (byte) (num5 >> 8);
        this.pending[this.pendingCount++] = (byte) num5;
        if (this.strstart != 0)
        {
      ...
      else if (this._codec.AvailableBytesIn == 0 && flush <= (FlushType) num1 && flush != FlushType.Finish)
        return 0;
