...
namespace Ionic.Crc
{
  ...
  public class CRC32
  {
    ...
    {
      return (int) this.crc32Table[(uint) (((int) W ^ (int) B) & (int) byte.MaxValue)] ^ (int) (W >> 8);
    }
    ...
    public void SlurpBlock(byte[] block, int offset, int count)
    {
      ...
      for (int index1 = 0; index1 < count; ++index1)
      {
        ...
        this._register = !this.reverseBits ? this._register >> 8 ^ this.crc32Table[(this._register & (uint) byte.MaxValue ^ (uint) num)] : this._register << 8 ^ this.crc32Table[(this._register >> 24 ^ (uint) num)];
      }
    ...
    {
      if (this.reverseBits)
        this._register = this._register << 8 ^ this.crc32Table[(this._register >> 24 ^ (uint) b)];
      else
        this._register = this._register >> 8 ^ this.crc32Table[(this._register & (uint) byte.MaxValue ^ (uint) b)];
    }
    ...
    public void UpdateCRC(byte b, int n)
    {
      while (n-- > 0)
      {
        if (this.reverseBits)
        {
          ...
          this._register = this._register << 8 ^ this.crc32Table[num >= 0U ? num : (num + 256U)];
        }
        else
        {
